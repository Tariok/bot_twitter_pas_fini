...
#thp= "@the_hacking_pro"
#five = journalist.sample(5)
#hello= "#bonjour_monde"
  #five.each do |i|
    #client.update("#{i} \nSalut, comment ça va ? Juste pour vous faire un coucou de la part de la part de #{thp} #{hello}")
  #end


  
# # Liker les 25 derniers tweet #bonjour_monde

# tweets = client.search("#bonjour_monde", result_type: "recent").take(25)

# client.favorite(tweets)

# client.search("#bonjour_monde", result_type: "recent").take(3).uniq.collect do |tweet|
#   client.follow(tweet.user.screen_name)
# end

# client.search("#bonjour_monde", result_type: "recent").uniq.collect do |tweet|
#   client.favorite(tweet)
#   client.follow(tweet.user.screen_name)
# end

# client.filter(track : #) do |tweet|
...
# sclient = Twitter::Streaming::Client.new do |config|         # topics to watch       
# config.consumer_key        = ENV["TWITTER_CONSUMER_KEY"]
# config.consumer_secret     = ENV["TWITTER_CONSUMER_SECRET"]
# config.access_token        = ENV["TWITTER_ACCESS_TOKEN"]
# config.access_token_secret = ENV["TWITTER_ACCESS_TOKEN_SECRET"]
...
# sclient.filter(:track => topics) do |tweet|             
#   if tweet.is_a?(Twitter::Tweet)               
#     puts tweet.text                
#     rclient.fav tweet             
#   end         
...
# end
config = [consumer_key :ENV["TWITTER_CONSUMER_KEY"] ,consumer_secret :ENV["TWITTER_CONSUMER_SECRET"],access_token :ENV["TWITTER_ACCESS_TOKEN"],access_token_secret :ENV["TWITTER_ACCESS_TOKEN_SECRET"]]
